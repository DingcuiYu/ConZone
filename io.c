...
static unsigned int __do_perform_io_using_dma(int sqid, int sq_entry)
{
	...
	while (remaining) {
		...
		/* For non-page aligned paddr, it will never be between continuous PRP list (Always first
		 * paddr)  */
		if (paddr & PAGE_OFFSET_MASK) {
			mem_offs = paddr & PAGE_OFFSET_MASK;
	...
	while (!kthread_should_stop()) {
	 ...
	 SUPPORTED_SSD_TYPE(CONZONE_BLOCK) || SUPPORTED_SSD_TYPE(CONZONE_META))
				...
				trace_printk("%llu %llu %llu %llu %llu %llu\n", w->nsecs_start,
							 ...
							 w->nsecs_cq_filled - w->nsecs_start, w->nsecs_target - w->nsecs_start);
#endif
...
void NVMEV_IO_WORKER_INIT(struct nvmev_dev *nvmev_vdev)
{
	...
	for (worker_id = 0; worker_id < nvmev_vdev->config.nr_io_workers; worker_id++) {
		...
		worker->work_queue = kzalloc(sizeof(struct nvmev_io_work) * NR_MAX_PARALLEL_IO, GFP_KERNEL);
