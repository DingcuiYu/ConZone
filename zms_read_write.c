...
static int l2p_replace(struct zms_ftl *zms_ftl, uint64_t la, int gran, int res)
{
	...
	cache->mapping[slot][evict_idx].nsid = nsid;
...
static int l2p_insert(struct zms_ftl *zms_ftl, uint64_t la, int gran, int res)
{
	...
	cache->mapping[slot][idx].nsid = nsid;
...
static void get_new_wp(struct zms_ftl *zms_ftl, struct zms_write_pointer *wpp)
{
	...
	if (wpp == &zms_ftl->pslc_wp) {
		zms_ftl->line_write_cnt++;
		wpp->curline->mid.write_order = zms_ftl->line_write_cnt;
		pqueue_insert(zms_ftl->migrating_line_pq, &wpp->curline->mid);
	}
...
static void nextpage(struct zms_ftl *zms_ftl, struct ppa *ppa)
{
	struct zms_line *line = get_line(zms_ftl, ppa);
	if (line->parent_id == -1)
		nextpage_interleave(zms_ftl, ppa);
...
static inline bool should_migrate_high(struct zms_ftl *zms_ftl)
{
	return zms_ftl->lm.pslc_free_line_cnt <= zms_ftl->zp.migrate_thres_lines_high;
}
...
static inline void consume_write_credit(struct zms_ftl *zms_ftl, bool pSLC, int pgs)
{
	if (pSLC == LOC_PSLC) {
		zms_ftl->pslc_wfc.write_credits -= pgs;
	} else {
		zms_ftl->wfc.write_credits -= pgs;
	}
}
...
static void submit_internal_write(struct zms_ftl *zms_ftl, struct zms_line *line, int io_type)
{
	...
	if (agg_len > 0) {
		if (io_type == GC_IO && dest_loc == LOC_NORMAL) {
			...
			for (int i = 0; i < agg_len; i++) {
				uint64_t lpn = agg_lpns[i];
				int migrate_agg_len = zms_ftl->zone_agg_pgs[0];
				zms_ftl->zone_agg_lpns[0][migrate_agg_len] = lpn;
				zms_ftl->zone_agg_pgs[0]++;

				if (zms_ftl->zone_agg_pgs[0] == pgs_per_oneshotpg) {
					internal_write(zms_ftl, zms_ftl->zone_agg_lpns[0], 0, zms_ftl->zone_agg_pgs[0],
								   io_type, dest_loc, 0);
					zms_ftl->zone_agg_pgs[0] = 0;
					sidx = i + 1;
				}
			}
			if (sidx < agg_len) {
				internal_write(zms_ftl, agg_lpns, sidx, agg_len, io_type, LOC_PSLC, 0);
			}
		} else {
			internal_write(zms_ftl, agg_lpns, 0, agg_len, io_type, LOC_PSLC, 0);
		}
...
static struct zms_line *do_migrate(struct zms_ftl *zms_ftl, int io_type)
{
	...
	// NVMEV_ZMS_GC_DEBUG
	NVMEV_INFO("get migrate line id: %d vpc: %d ipc: %d rpc :%d\n", sblk_line->id, sblk_line->vpc,
			   sblk_line->ipc, sblk_line->rpc);
	...
	NVMEV_INFO("migrate end line id: %d  vpc: %d ipc: %d rpc:%d\n", sblk_line->id, sblk_line->vpc,
			   sblk_line->ipc, sblk_line->rpc);
	return sblk_line;
}
...
static bool __zms_wb_hit(struct zms_ftl *zms_ftl, struct buffer *write_buffer, uint64_t lpn)
{
	if (zms_ftl->zp.ns_type == SSD_TYPE_ZMS_ZONED) {
		...
		for (int i = 0; i < write_buffer->pgs; i++) {
			if (write_buffer->lpns[i] == lpn && write_buffer->nsid == zms_ftl->zp.ns->id)
				return true;
		}
		return false;
	}
								 ...
								 struct nvmev_result *ret)
{
	...
	// 好像不能让 ” Need to increase array size“ 不产生
	// if (zms_ftl->zp.ns_type != SSD_TYPE_ZMS_META && !SLC_BYPASS && should_migrate_high(zms_ftl))
	// { 	NVMEV_ZMS_RW_DEBUG_VERBOSE( 		"Waiting for migrating... current time %lld us max
	// free %lld us\n", 		zms_ftl->current_time / 1000, check_maxfreetime(zms_ftl) / 1000);
	// 	try_migrate(zms_ftl);
	// 	return false;
	// }
